...
#define round 3  //number of chains
#define run_num 1
int main(int argc, char **argv) 
{
    ...
    uint8_t  *inputbuf_data  = (uint8_t*) GMIO::malloc(round*2*mlen*sizeof(uint8_t)); 
    uint8_t *outputbuf_data = (uint8_t*) GMIO::malloc(round*32*sizeof(uint8_t));
    uint8_t  *inputbuf_addr  = (uint8_t*) GMIO::malloc(round*mlen*sizeof(uint8_t)); 
  ...
  printf("\nThe input data is :\n");
    ...
    for(int i=0;i<run_num;i++){
         //mygraph.update(mygraph.runtimes, round);
