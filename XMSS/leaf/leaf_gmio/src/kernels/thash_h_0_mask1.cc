...
void thash_h_0_mask1(input_stream<uint32> * __restrict bufin, input_stream<uint32> * __restrict prfin, /*int len , */ output_stream<uint32>* __restrict bufout,output_stream<uint32> * __restrict prfout)
{
    ...
    while(node_num <34){  // do mask 33 times and specially, the 34th node dont make any process and output straightfully
        ...
        for(int i=0;i<32;i++){
            writeincr(bufout, temp[i]); // output to H function immediately
        }
    ...
    //conversed input xor with conversed vector H
    ...
    //Reverse happens when transfer data from AIE to PS through DDR, thats,  ABCD -> DCBA， so I reverse it before sending
        ...
        for(int j=8;j<16;j++)//read prf from prf function
