...
void thash_h_5(input_stream<uint32> * __restrict data, input_stream<uint32> * __restrict addr, 
                /*unsigned char (&Pubseed)[32],*/ output_stream<uint32>* __restrict dout1, output_stream<uint32>* __restrict dout2)
{
    
    int wots_pk_num =0;
    while(wots_pk_num <3){ // totally 3 leaf nodes
    ...
    //accept the stream
        ...
        temp_addr[5]=0x05000000; //address height = 0 in the first layer   
    ...
    // if(wots_pk_num==1){
    //     printf("\nthash_h_l5——golden_datain\n");
    //     for(int i=0;i<8;i++){
    //         printf("%02x",datain[i]);
    //     }
    //     printf("\n");

    //     printf("\nthash_h_l5——golden__addr\n");
    //     for(int i=0;i<8;i++){
