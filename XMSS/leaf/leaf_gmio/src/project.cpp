...
int main(int argc, char **argv) 
{
    ...
    uint8_t  *inputbuf_data  = (uint8_t*) GMIO::malloc(3*mlen*sizeof(uint8_t)); //GMIO should be multiple of 32-bit
    uint8_t *outputbuf_data = (uint8_t*) GMIO::malloc(round*2*32*sizeof(uint8_t));

    //uint8_t  *inputbuf_addr  = (uint8_t*) GMIO::malloc(round*mlen*sizeof(uint8_t)); //GMIO should be multiple of 32-bit
...
//   printf("\nThe pubseed is :\n");
   ...
   //inputbuf_data[83]=255;//OTS addr 
    ...
    for(int i=0;i<run_num;i++){
        mygraph.datain.gm2aie_nb(&inputbuf_data[0],3*mlen*sizeof(uint8_t));
        mygraph.datain.wait();
