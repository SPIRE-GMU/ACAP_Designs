...
{
    uint32_t temp[16]; //din ||prf
    uint32_t temp_mask[24];// mask2 || pubseed || addr
    unsigned char buf[128];//;
    unsigned char out[32];

    int run_num =0;

    while(run_num < 34){ //recieve 34 input data
    
    if(run_num==33){

        uint32_t temp[32];
        for(int j=0;j<32;j++)//prf || pub || addr || datain
        ...
        for(int i=0;i<8;i++){
            copy_u32_u8(buf+32+i*4, temp[8+i]); //prf
            copy_u32_u8(buf+64+i*4, temp[i]);//mask 1
            copy_u32_u8(buf+96+i*4, temp_mask[i]);//mask 2
        }
