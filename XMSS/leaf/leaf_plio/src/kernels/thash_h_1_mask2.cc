...
#define copy_uint32(p, val) *((uint32_t *)p) = __builtin_bswap32((val))//gcc__builtin_bswap32，
...
void thash_h_1_mask2(input_stream<uint32> * __restrict bufin,  /*int len , */ output_stream<uint32>* __restrict bufout)
{
    ...
    uint32_t temp[32]; //pad ||pubseed | addr || datain
    
    int node_num =0;
    while(node_num <17){  // only 33 nodes come
    ...
    for(int i=0;i<8;i++){
            writeincr(bufout, fill(out+i*4)^temp[i+24]); //output data
        }

   for(int i=8;i<24;i++){
        writeincr(bufout, temp[i]); //output pubseed and addr mask2
   }
