...
void wots_sk_gen(input_stream<uint32> * __restrict bufin, output_stream<uint32>* __restrict bufout, output_stream<uint32>* __restrict addr_out)
{
    ...
    uint32_t temp[24]; //pubseed || sk_seed || addr
    ...
    for(int j=0;j<24;j++)//
        ...
        {  
           ...
           if(j>7){
                copy_u32_u8(buf+j*4, temp[j]);  // prf || sk_seed || addr
           }
    ...
    // printf("\nbuf=");
    // for(int i=0;i<24;i++)
    //     printf("%02x",fill(buf+i*4));
    
    sha256(buf,96,out); // get the seed for wots sk
    // printf("\nh=");
    //     printf("%02x",fill(out));
    copy_u8_u8(buf+32, out, 32); // prf || seed || addr ( ctr)
    buf[83]=0; //change wots addr to 0 and expand wots_sk
    buf[82]=0;
    buf[81]=0;
    buf[80]=0;

    for(int i=0;i<67;i++)chess_prepare_for_pipelining{ //compute wots sk and output
        // printf("\nbuf=");
        // for(int i=0;i<24;i++)
            // printf("%02x",fill(buf+i*4));
        sha256(buf,96,out);
        ...
        // printf("%02x",fill(out));
        for(int i=0;i<8;i++){
            writeincr(bufout, fill(out+i*4)); //output wots-sk  
            
        }
        for(int i=0;i<8;i++){
            writeincr(bufout, temp[i]); //output pubseed
