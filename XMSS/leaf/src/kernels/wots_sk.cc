...
void wots_sk_gen(input_stream<uint32> * __restrict bufin, output_stream<uint32>* __restrict bufout, output_stream<uint32>* __restrict addr_out)
{
    ...
    //int len=96;  defined in the head
   ...
   //unsigned long long time1=tile.cycles();
    ...
    uint32_t temp[len/4]; //we read 32-bit in one cycle，and len infers to the length of char
    ...
    if (len > 0) {
        memcpy(buf+32, temp+8, 64); //   insert sk_seed   readincr is conversed,but corrected here : DCBA -> ABCD                                                                 
    }
