...
private:
  kernel thash_f1;  
  kernel sha2561;
  kernel sha2562; //test one stream output to two input stream
  kernel sha256_mask1;
  kernel sha256_f1;
...
public:
  ...
  simpleGraph()
  {
    ...
    // datain = input_plio::create(plio_32_bits, "data/input_1.txt");
    // addrin = input_plio::create(plio_32_bits, "data/input_1.txt");
    
    out0 = output_plio::create(plio_32_bits, "data/output_1.txt");
    // dataout = output_plio::create(plio_32_bits, "data/data_output_1.txt");
    // addrout = output_plio::create(plio_32_bits, "data/addr_output_1.txt");
    ...
    connect<stream> net8 (thash_f1.out[0], sha2562.in[0]);
    connect<stream> net9 (sha2562.out[0], out0.in[0]);
