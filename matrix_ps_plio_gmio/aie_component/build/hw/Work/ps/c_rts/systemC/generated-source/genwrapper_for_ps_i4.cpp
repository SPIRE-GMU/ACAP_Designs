...
std::map<std::string, adf::GMIO*> gmioMap = { 
	{"mygraph.datain1", &mygraph.datain1},
	{"mygraph.datain2", &mygraph.datain2},
	{"mygraph.dataout", &mygraph.dataout}
};
...
class PSIP_ps_i4 : public IPBlock {
	 SC_HAS_PROCESS(PSIP_ps_i4);
...
public:
	 ...
	 static PSIP_ps_i4* createInstance(sc_module_name name);
	 //  Returns the instance created using createInstance()
	 static PSIP_ps_i4* getInstance();
...
private:
	 PSIP_ps_i4(sc_module_name nm);
	 static PSIP_ps_i4* psObj;
...
PSIP_ps_i4* PSIP_ps_i4::psObj = NULL;
PSIP_ps_i4* PSIP_ps_i4::createInstance(sc_module_name name) {
	 if (psObj == NULL)
		 psObj = new PSIP_ps_i4(name);
	 return psObj;
}

PSIP_ps_i4* PSIP_ps_i4::getInstance() {
	 if (psObj != NULL) {
		 return psObj;
	 } else {
...
void PSIP_ps_i4::write32(uint64_t Addr, uint32_t Data) {
...
uint32_t PSIP_ps_i4::read32(uint64_t Addr) {
	 size_t transBytes = sizeof(uint32_t);
	 // Get the payload object
	 xtlm::aximm_payload* trans = mem_manager->get_payload();
	 trans->acquire();
...
void PSIP_ps_i4::write128(uint64_t Addr, uint32_t* Data) {
...
void PSIP_ps_i4::read128(uint64_t Addr, uint32_t* Data) {
	 size_t transBytes = 4*sizeof(uint32_t);
	 // Get the payload object
	 xtlm::aximm_payload* trans = mem_manager->get_payload();
	 trans->acquire();
...
void PSIP_ps_i4::writeGM(uint64_t addr, const void* data, uint64_t size) {
    toggle_AIE_array_clk.notify(1,SC_NS);
    uint64_t remaining = size;
    uint64_t currentAddr = addr;
    unsigned char* currentDataPtr = (unsigned char*)data;
    unsigned int trans_size_in_multiple_of_16bytes = (size + 15) / 16;
    ...
    payload->set_command(xtlm::XTLM_WRITE_COMMAND); // write
    payload->set_data_ptr((unsigned char*)data, size); //caller manages data memory 
    payload->set_address(addr); //DDR address 
    ...
    unsigned int ret_val = PS_AxiMM_Wr_Util->transport_dbg(*payload);
    if(ret_val != 0) {
      payload->release();
     }
     else {
...
void PSIP_ps_i4::readGM(uint64_t addr, void* data, uint64_t size) {
    toggle_AIE_array_clk.notify(1,SC_NS);
    uint64_t remaining = size;
    uint64_t currentAddr = addr;
    unsigned char* currentDataPtr = (unsigned char*)data;
    ...
    payload->acquire();
    payload->set_response_status(xtlm::XTLM_INCOMPLETE_RESPONSE);
    payload->set_command(xtlm::XTLM_READ_COMMAND); //set read command 
    payload->set_data_ptr((unsigned char*)data, size); //caller manages data memory
    payload->set_address(addr); //DDR address 
    payload->set_burst_type(1); //1:INCR to allow burst length to reach maximum 256
    payload->set_burst_length(trans_size_in_multiple_of_16bytes); //burst length, maximum 256
    payload->set_burst_size(16); //128-bit (16-byte) burst size (buswidth)
    unsigned int ret_val = PS_AxiMM_Rd_Util->transport_dbg(*payload);
    if(ret_val != 0) {
      payload->release();
    } 
    else 
...
void PSIP_ps_i4::aximm_transaction(xtlm::xtlm_aximm_initiator_rd_socket_util& rd_util, xtlm::xtlm_aximm_initiator_wr_socket_util& wr_util, xtlm::xtlm_command command, unsigned long long address, unsigned char* pData, unsigned int trans_size_in_bytes)
{
...
void PSIP_ps_i4::main_action() {
	 ...
	 adf::setWorkDirectory(g_pkg_dir);
#ifdef _HYBRID_EMU_
	 if(is_hybrid_emu()) {
		 xrt_ps_main();
	 } else
...
void PSIP_ps_i4::set_payload_attr(xtlm::aximm_payload* trans, size_t transBytes) {
...
void PSIP_ps_i4::response_process() {
	 while (true) {
...
extern "C" {
	 void ess_Write32(uint64 Addr, uint Data) {
		  (PSIP_ps_i4::getInstance())->write32(Addr, Data); 
	 }
	 uint32 ess_Read32(uint64 Addr) {
		  return ((PSIP_ps_i4::getInstance())->read32(Addr));
	 }
	 void ess_Write128(uint64_t Addr, uint32_t* Data) {
		  (PSIP_ps_i4::getInstance())->write128(Addr, Data);
	 }
	 void ess_Read128(uint64_t Addr, uint32_t* Data) {
		  (PSIP_ps_i4::getInstance())->read128(Addr, Data);
	 }
	 void ess_WriteGM(uint64 addr, const void* data, uint64_t size) {
		  (PSIP_ps_i4::getInstance())->writeGM(addr, data, size);
	 }
	 void ess_ReadGM(uint64 addr, void* data, uint64_t size) {
		  (PSIP_ps_i4::getInstance())->readGM(addr, data, size);
	 }
	 IPBlock * create_ip(sc_module_name name) {
		  return (PSIP_ps_i4::createInstance(name));
	 }
	 void destroy_ip(IPBlock* ip) {
		  std::cout << "IP-INFO: deleting ip PSIP_ps_i4 " << std::endl;
		  delete ip;
	 }
}
